...
import models.gaussian_diffusion_imp as gd_imp
...
class Diffrunner(object):
    ...
    def evaluate_imp(self,data_loader, data_te, mask_his, topN,excluded_items_per_user):
        # print(len(excluded_items_per_user))
        # print(mask_his.shape[0])
        # print(f'fuyangben {len(excluded_items_per_user[0])}')
        os.environ["CUDA_VISIBLE_DEVICES"] = self.gpu
    ...
    def train(self,data_dict):
        ...
        in_dims = out_dims[::-1]
        self.model = DNN(in_dims, out_dims, self.emb_size, time_type="cat", norm=self.norm).to(device)
        ...
        try:
            for epoch in range(0, self.epoch ):
                ...
                dev_result = self.evaluate(test_loader, valid_y_data, train_data, self.topN,neg_validlist)
                ...
                if self.test_epoch > 0 and epoch % self.test_epoch  == 0:
                    test_result = self.evaluate(test_loader, test_y_data, train_data, self.topN,neg_testlist)
    ...
    def print_res(self, dataset: BaseModel.Dataset) -> str:
        ...
        if self.printflag==0 :
            ...
            result_dict = self.evaluate(self.test_loader, self.valid_y_data, self.train_data, self.topN,self.neg_validlist)
        else:
            result_dict = self.evaluate(self.test_loader, self.test_y_data, self.train_data, self.topN,self.neg_validlist)
        res_str = '(' + utils.format_metric(result_dict) + ')'
